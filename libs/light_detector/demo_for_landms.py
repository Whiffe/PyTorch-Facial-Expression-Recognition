...
from utils import image_utils, file_utils, debug
...
class Detector(demo.Detector):
    ...
    def detect_image_dir(self, image_dir, isshow=True):
        ...
        :return:
        """
        image_list = file_utils.read_files_lists(image_dir)
    ...
    def show_landmark_boxes(win_name, image, boxes, labels, probs, landms):
        ...
        rgb_image = image_utils.draw_landmark(image, landms, color=(255, 0, 0), vis_id=False)
        rgb_image = image_utils.draw_image_detection_bboxes(rgb_image, boxes, probs, labels, color=(0, 0, 255))
    ...
    def show_landmark_boxes1(win_name, image, boxes, labels, probs, landms):
        ...
        for i in range(len(boxes)):
            print(landms[i])
            rgb_image = image_utils.draw_landmark(image, [landms[i]], vis_id=True)
            rgb_image = image_utils.draw_image_detection_bboxes(rgb_image,
                                                                [boxes[i]],
                                                                [probs[i]],
                                                                [labels[i]])
            bgr_image = cv2.cvtColor(rgb_image, cv2.COLOR_RGB2BGR)
