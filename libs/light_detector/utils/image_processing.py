...
def pointPolygonTest(point, contour, measureDist=False):
    ...
    :param measureDist: 如果为True，则查找签名距离.
...
def draw_image_boxes(bgr_image, boxes_list, color=(0, 0, 255), thickness=2):
...
def draw_image_detection_rects(rgb_image, rects, probs, lables, boxes_name=None, color=None):
    bboxes = rects2bboxes(rects)
    rgb_image = draw_image_detection_bboxes(rgb_image, bboxes, probs, lables, boxes_name, color)
    return rgb_image
...
def draw_image_detection_bboxes(rgb_image, bboxes, probs, labels, boxes_name=None, color=None):
    '''
    :param title:
    :param rgb_image:
...
def show_image_detection_bboxes(title, rgb_image, bboxes, probs, lables, color=None, waitKey=0):
    rgb_image = draw_image_detection_bboxes(rgb_image, bboxes, probs, lables, color=color)
    cv_show_image(title, rgb_image, waitKey=waitKey)
    return rgb_image
...
def draw_key_point_in_image(image, key_points, pointline=[], color=None):
    '''
...
def draw_image_points_lines(image, points, pointline=[], texts=None, drawType="simple", ):
    '''
    在图像中画点和连接线
    :param img:
    :param points: 点列表
    ...
    draw_image_lines(image, points, pointline, thickness=thickness)
    return image
...
if __name__ == "__main__":
    ...
    show_image_boxes("image1", image1, boxes1)
